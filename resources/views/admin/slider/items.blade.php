  ...
  $(document).on('click', '.edit-item-button', function(){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        $('#itemModal img').attr('src', data.five_preview);
...
@include('layouts.admin.header', ['title' => 'Слайдер', 'content' => 'Редактирование слайдера на главной странице'])
...
@else 
  @foreach($items as $ki => $item)
          <tr id="item-{{$item->id }}" >
            <td class="item-position @if($loop->index == 0) item-position-first @endif" data-position="{{ $item->position }}" data-column="position" data-table="items" data-id="{{ $item->id }}">
...
@include('layouts.admin.form.form-input', ['title' => __('Title'), 'name' => 'title', 'value' => NULL])

@include('layouts.admin.form.form-textarea', ['title' => __('Description'), 'name' => 'text1'])

@include('layouts.admin.form.form-input', ['title' => __('Link'), 'name' => 'nol1', 'languages' => false])
